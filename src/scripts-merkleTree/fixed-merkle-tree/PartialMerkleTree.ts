...
export class PartialMerkleTree extends BaseTree {
  ...
  constructor(
    levels: number,
    { edgePath, edgeElement, edgeIndex, edgeElementsCount }: TreeEdge,
    leaves: Element[],
    { hashFunction, zeroElement }: MerkleTreeOptions = {}
  ) {
    super();
  ...
  update(index: number, element: Element) {
    ...
    if (index < this._edgeLeaf.index) {
      throw new Error(
        `Index ${index} is below the edge: ${this._edgeLeaf.index}`
      );
    }
  ...
  path(index: number): ProofPath {
    ...
    for (let level = 0; level < this.levels; level++) {
      ...
      pathElements[level] =
        pathElements[level] ??
        (proofPos === leafIndex ? proofEl : this._zeros[level]);
      elIndex >>= 1;
    }
    return {
      pathElements,
  ...
  shiftEdge(edge: TreeEdge, elements: Element[]) {
    if (this._edgeLeaf.index <= edge.edgeIndex) {
      throw new Error(
        `New edgeIndex should be smaller then ${this._edgeLeaf.index}`
      );
    }
    if (elements.length !== this._edgeLeaf.index - edge.edgeIndex) {
      throw new Error(
        `Elements length should be ${this._edgeLeaf.index - edge.edgeIndex}`
      );
    }
  ...
  static deserialize(
    data: SerializedPartialTreeState,
    hashFunction?: HashFunction<Element>
  ): PartialMerkleTree {
    const instance: PartialMerkleTree = Object.assign(
